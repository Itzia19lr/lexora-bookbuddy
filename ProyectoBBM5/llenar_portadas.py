...
# ── Portadas SVG ─────────────────────────────────────────────
COLORES_GENERO = {
    "fantasy":{"bg":"#2d1b69","accent":"#a78bfa","label":"Fantasy"},
    "thriller":{"bg":"#111111","accent":"#ef4444","label":"Thriller"},
    "mystery":{"bg":"#1f2937","accent":"#60a5fa","label":"Mystery"},
    "romance":{"bg":"#9f1239","accent":"#fda4af","label":"Romance"},
    "horror":{"bg":"#0a0a0a","accent":"#ff4d4d","label":"Horror"},
    "science fiction":{"bg":"#0f172a","accent":"#38bdf8","label":"Sci-Fi"},
    "science+fiction":{"bg":"#0f172a","accent":"#38bdf8","label":"Sci-Fi"},
    "sci-fi":{"bg":"#0f172a","accent":"#38bdf8","label":"Sci-Fi"},
    "fiction":{"bg":"#1e3a5f","accent":"#93c5fd","label":"Ficción"},
    "literary fiction":{"bg":"#1e3a5f","accent":"#93c5fd","label":"Literaria"},
    "contemporary":{"bg":"#0f3460","accent":"#e94560","label":"Contemporáneo"},
    "contemporary fiction":{"bg":"#0f3460","accent":"#e94560","label":"Contemporáneo"},
    "historical fiction":{"bg":"#44271f","accent":"#f59e0b","label":"Hist. Ficción"},
    "historical+fiction":{"bg":"#44271f","accent":"#f59e0b","label":"Hist. Ficción"},
    "adventure":{"bg":"#14532d","accent":"#4ade80","label":"Aventura"},
    "young adult":{"bg":"#4a1d96","accent":"#f0abfc","label":"Young Adult"},
    "young+adult":{"bg":"#4a1d96","accent":"#f0abfc","label":"Young Adult"},
    "ya":{"bg":"#4a1d96","accent":"#f0abfc","label":"YA"},
    ...
    "true crime":{"bg":"#1a1a2e","accent":"#e94560","label":"True Crime"},
    "true+crime":{"bg":"#1a1a2e","accent":"#e94560","label":"True Crime"},
    ...
    "childrens":{"bg":"#5b21b6","accent":"#fdba74","label":"Infantil"},
...
div[data-testid="stButton"] > button:hover { background: #f6121d !important; }
...
.arrow-btn div[data-testid="stButton"] > button:hover { background: rgba(255,255,255,0.3) !important; }
[data-testid="stProgressBar"] > div > div > div > div { background-color: #E50914; }
...
@st.cache_data
def cargar_datos():
    libros   = pd.read_csv('data/libros.csv', encoding='latin-1')
    usuarios = pd.read_csv('data/usuarios.csv', encoding='latin-1')
    ratings  = pd.read_csv('data/ratings.csv', encoding='latin-1')
    pop      = pd.read_csv('data/popularidad_libros.csv', encoding='latin-1')

    # Limpiar nombres de columnas SIN quitar caracteres especiales
    # para preservar 'año', 'paginas', etc.
    for df in [libros, usuarios, ratings, pop]:
        df.columns = [c.strip() for c in df.columns]

    with open('data/generos.json')       as f: g  = json.load(f)

    # Intentar abrir stats con y sin ñ
    try:
    ...
    try:
        ...
        try:
            with open('data/stats_años.json', encoding='utf-8') as f: sa = json.load(f)
        except:
            sa = {}
...
# Índice book_id → fila garantizado con reset_index
_libros_reset = libros_df.reset_index(drop=True)
bookid_to_row = {row['book_id']: idx for idx, row in _libros_reset.iterrows()}

# Detectar columna de año una sola vez al cargar
_COL_ANO = None
for _c in _libros_reset.columns:
    _cl = _c.strip().lower()
    if _cl in ('año', 'ano', 'año_publicacion', 'ano_publicacion',
                'a\xf1o', 'a\xf1o_publicacion'):
        _COL_ANO = _c
        break

def get_año(libro):
    """Obtiene el año sin importar el nombre exacto de la columna."""
    # Intentar con la columna detectada al inicio
    if _COL_ANO:
        val = None
        if isinstance(libro, pd.Series):
            val = libro.get(_COL_ANO)
        else:
            val = getattr(libro, _COL_ANO, None)
        if val is not None and pd.notna(val):
            try: return int(float(val))
            except: pass

    # Fallback: probar todos los nombres posibles
    for col in ['año_publicacion', 'ano_publicacion', 'año', 'ano',
                 'a\xf1o_publicacion', 'a\xf1o']:
        val = None
        if isinstance(libro, pd.Series):
            val = libro.get(col)
        else:
            val = getattr(libro, col, None)
        if val is not None and pd.notna(val):
            try: return int(float(val))
            except: pass
    return "—"
...
def get_col_ano_df(df):
    """Devuelve el nombre de la columna de año en un DataFrame."""
    for c in df.columns:
        if c.strip().lower() in ('año', 'ano', 'año_publicacion', 'ano_publicacion',
                                  'a\xf1o', 'a\xf1o_publicacion'):
            return c
    return None

# ── Logo para páginas internas ────────────────────────────────
...
GENERO_ES = {
    ...
    "fiction":"Ficción","literary fiction":"Ficción Literaria",
    "contemporary":"Contemporáneo","contemporary fiction":"Ficción Contemporánea",
    ...
    "ya":"YA","dystopia":"Distopía","graphic novel":"Novela Gráfica",
    "classics":"Clásicos","poetry":"Poesía","short stories":"Cuentos Cortos",
    "history":"Historia","philosophy":"Filosofía","psychology":"Psicología",
    "biography":"Biografía","autobiography":"Autobiografía","business":"Negocios",
    "memoir":"Memorias","self help":"Autoayuda","self-help":"Autoayuda",
    "science":"Ciencia","nonfiction":"No Ficción","non-fiction":"No Ficción",
    "true crime":"Crimen Real","true+crime":"Crimen Real","travel":"Viajes",
    "cooking":"Cocina","art":"Arte","religion":"Religión","humor":"Humor",
    "children":"Infantil","childrens":"Infantil","politics":"Política","economics":"Economía",
}
...
# ── Recomendación ─────────────────────────────────────────────
def recomendar_hibrido(pref, top_n=3):
    df = _libros_reset.copy()

    if pref['generos']:
        df = df[df['genero'].isin(pref['generos'])]

    # Filtro páginas — incluye libros sin dato en medianos y largos
    if pref['paginas'] == 'Cortos (menos de 300 páginas)':
        df = df[df['paginas'].fillna(9999) < 300]
    elif pref['paginas'] == 'Medianos (300-500 páginas)':
        df = df[
            df['paginas'].isna() |
            ((df['paginas'] >= 300) & (df['paginas'] <= 500))
        ]
    ...
    # 'No me importa la extensión' → no filtra

    # Filtro época — detectar columna año dinámicamente
    ...
    if col_ano:
        ...
        # 'No tengo preferencia' → no filtra

    pop = df.merge(popularidad_df, on='book_id', how='left')

    if pref['popularidad'] == 'Bestsellers y libros populares':
        pop = pop.nlargest(150, 'num_ratings')
    elif pref['popularidad'] == 'Joyas escondidas poco conocidas':
        pop = pop.nsmallest(150, 'num_ratings')

    if len(pop) == 0:
        res = _libros_reset.sample(min(top_n, len(_libros_reset))).copy()
        res['score'] = np.random.uniform(0.75, 0.95, size=len(res))
        return res

    ids = pop['book_id'].tolist()
    sid = pop.iloc[0]['book_id']

    try:
        i0 = bookid_to_row.get(sid)
        if i0 is None:
            raise ValueError(f"book_id {sid} no encontrado")
        ...
        for bid in ids:
            ...
            if i is not None:
                scores[bid] = float(sims[i]) + random.uniform(-0.08, 0.08)

        top = sorted(scores.items(), key=lambda x: x[1], reverse=True)[:top_n]
        res = _libros_reset[_libros_reset['book_id'].isin([x[0] for x in top])].copy()
        res['score'] = res['book_id'].map(dict(top))
        res = res.sort_values('score', ascending=False).merge(popularidad_df, on='book_id', how='left')
        return res.head(top_n)

    except Exception:
        fb = pop.sample(min(top_n, len(pop))).copy()
        fb['score'] = np.linspace(0.90, 0.75, len(fb))
        return fb

def generar_explicacion(libro, pref):
    if not API_KEY or not client:
        g = getattr(libro, 'genero', None)
        return f"Ideal para fans del {g}." if g in pref.get('generos', []) else "Coincide con tu perfil lector."
    prompt = (f'En UNA oración (máx 15 palabras), explica por qué "{libro.titulo}" '
              f'encaja con alguien que busca {pref["paginas"].lower()} de {libro.genero}. Español de México.')
    try:
        r = client.chat.completions.create(
            model="gpt-3.5-turbo",
        ...
        return "Coincide con tu perfil lector."
...
def mostrar_home():
    ...
    if FONDO_B64:
        ...
        bg_style = "background:linear-gradient(135deg,#1a0808 0%,#0a0a1a 100%);"

    logo_hero = (f'<img src="data:image/png;base64,{LOGO_B64}" height="48">'
                 if LOGO_B64 else '<span style="font-size:1.8rem;font-weight:900;color:white;">📚 Lexora</span>')
    ...
    for col, (_, libro) in zip([c1,c2,c3,c4,c5], visibles.iterrows()):
        ...
        with col:
            ...
            st.markdown(f'<div class="poster-author">{str(libro["autor"])[:26]}</div>', unsafe_allow_html=True)
...
def mostrar_catalogo():
    ...
    with col_b:
        if st.button("⬅ Inicio"): st.session_state.pagina = 'home'; st.rerun()

    c1, c2, c3 = st.columns(3)
    with c1: gf = st.selectbox("Género", ["Todos"] + sorted(_libros_reset['genero'].dropna().unique().tolist()))
    with c2: ab = st.text_input("Buscar autor")
    with c3: tb = st.text_input("Buscar título")

    df = _libros_reset.copy()
    if gf != "Todos": df = df[df['genero'] == gf]
    ...
    for idx, libro in enumerate(df.itertuples(index=False), start=1):
        with cols[(idx-1) % 5]:
            ...
            t = str(libro.titulo)
            st.markdown(f'<div class="poster-title">{t[:32]}{"…" if len(t)>32 else ""}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="poster-author">{libro.autor}</div>', unsafe_allow_html=True)
...
def mostrar_detalle():
    ...
    with c2:
        ...
        st.markdown(f"**{libro['autor']}**")
        st.divider()
        m1, m2, m3 = st.columns(3)
        with m1: st.metric("Género", str(libro.get('genero','')).title())
        with m2:
            pags = libro.get('paginas')
            st.metric("Páginas", int(pags) if pd.notna(pags) else "—")
        with m3:
            st.metric("Año", get_año(libro))
        st.divider()
        ...
        ca, cb2 = st.columns(2)
        with ca:
            if st.button("✨ Recomendaciones parecidas", use_container_width=True):
                st.session_state.pagina = 'preguntas'; st.rerun()
        with cb2:
            if st.button("📚 Seguir explorando", use_container_width=True):
                st.session_state.pagina = 'catalogo'; st.rerun()
...
def mostrar_preguntas():
    ...
    with st.form("form_pref"):
        st.markdown("### 1️⃣ ¿Cómo describirías tu hábito de lectura?")
        tl = st.radio("hab", ["Soy nuevo en la lectura","Leo ocasionalmente","Leo con frecuencia"], label_visibility="collapsed")
        st.markdown("### 2️⃣ ¿De qué época prefieres los libros?")
        ep = st.radio("ep", ["Clásicos (antes del 2000)","Contemporáneos (2000-2015)","Recientes (últimos años)","No tengo preferencia"], label_visibility="collapsed")
        st.markdown("### 3️⃣ ¿Qué extensión de libro prefieres?")
        pg = st.radio("pg", ["Cortos (menos de 300 páginas)","Medianos (300-500 páginas)","Largos (más de 500 páginas)","No me importa la extensión"], label_visibility="collapsed")
        st.markdown("### 4️⃣ ¿Qué géneros te interesan? (mínimo 2)")
        gc1, gc2 = st.columns(2)
        gs = []
        with gc1:
            st.markdown("**📚 FICCIÓN**")
            for g in ['fantasy','thriller','mystery','romance','horror','science+fiction']:
                if st.checkbox(g.replace('+',' ').title(), key=f"g_{g}"): gs.append(g)
        with gc2:
            st.markdown("**📖 NO FICCIÓN**")
            for g in ['history','philosophy','psychology','biography','business','memoir']:
                if st.checkbox(g.title(), key=f"g_{g}"): gs.append(g)
        st.markdown("### 5️⃣ ¿Qué tipo de libros buscas?")
        po = st.radio("po", ["Bestsellers y libros populares","Joyas escondidas poco conocidas","Un mix de ambos"], label_visibility="collapsed")

        if st.form_submit_button("🎯 Ver mis recomendaciones", use_container_width=True):
            if len(gs) < 2:
                st.error("⚠️ Selecciona al menos 2 géneros")
            else:
                ...
                with st.spinner("✨ Generando recomendaciones..."):
                    st.session_state.recomendaciones = recomendar_hibrido(st.session_state.preferencias, top_n=3)
...
def mostrar_resultados():
    ...
    if st.session_state.recomendaciones is None or len(st.session_state.recomendaciones) == 0:
        st.error("No encontramos libros con esos filtros. Intenta cambiar géneros, época o extensión.")
        if st.button("🔁 Cambiar preferencias"):
            st.session_state.pagina = 'preguntas'; st.rerun()
        return

    res = st.session_state.recomendaciones.head(3).copy().reset_index(drop=True)
    n_encontrados = len(res)
    st.markdown('<div class="section-title">🎯 Tus recomendaciones</div>', unsafe_allow_html=True)

    if n_encontrados < 3:
        st.warning(
            f"⚠️ Tu búsqueda es muy específica — solo encontramos **{n_encontrados} "
            f"{'libro' if n_encontrados==1 else 'libros'}** que cumple"
            f"{'n' if n_encontrados>1 else ''} exactamente con tus filtros. "
            f"Si quieres más opciones, prueba ajustando la época o extensión."
        )
    ...
    for idx, libro in enumerate(res.itertuples(index=False), start=1):
        with cols[idx-1]:
            ...
            st.markdown(f'<div class="poster-title">{t[:32]}{"…" if len(t)>32 else ""}</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="poster-author">{libro.autor}</div>', unsafe_allow_html=True)
            ms = int(float(libro.score)*100 if hasattr(libro,'score') and pd.notna(libro.score) else 85+random.randint(-5,10))
            st.progress(max(70, min(ms, 99)) / 100)
            with st.expander("Ver más"):
                ...
                with st.spinner("Traduciendo..."): st.write(traducir_descripcion(desc))
                st.info(f"💡 {generar_explicacion(libro, st.session_state.preferencias)}")

    st.divider()
    r1, r2, r3 = st.columns(3)
    with r1:
        if st.button("🔄 Otras opciones", use_container_width=True):
